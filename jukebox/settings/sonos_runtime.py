...
class SoCoSonosGroupResolver:
    def resolve_selected_group(
        ...
        for saved_member in selected_group.members:
            ...
            if resolved_speaker is None and saved_member.last_known_host is not None:
                try:
            ...
            resolved_members.append(
                ResolvedSonosSpeakerRuntime(
                    uid=resolved_speaker.uid,
                    name=resolved_speaker.player_name,
                    host=resolved_speaker.ip_address,
                    household_id=resolved_speaker.household_id,
                )
            )
    ...
    def _discover_available_speakers(soco_module: Any) -> dict[str, "_SonosSpeakerLike"]:
        ...
        return {speaker.uid: speaker for speaker in available_speakers}
